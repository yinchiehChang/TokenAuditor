...
contract CirculateBUSD  is Ownable, ReentrancyGuard{
    ...
    function IsNeededLiquidation(address _trader) public view returns (bool) {
        address _swappedToken = debtors[_trader].swappedToken;
        (,,,,,uint256 DailyLoanInterestRate,uint256 LiquidationLimitRate,) = ISwapHelper(SwapHelper).TradingInfo( BUSDContract,_swappedToken );
        if( debtors[_trader].tradingState == false )
            return false;
        uint256 estSwapOut = ISwapHelper(SwapHelper).getSwapOut(debtors[_trader].swappedToken , BUSDContract, debtors[_trader].swappedAmount );
    ...
    function setAutoStartTrading( uint256 _borrowAmount, address _swappedToken, uint256 swappedDownLimit, uint256 swappedUpLimit ) external {
        ...
        require( _borrowAmount <= _borrowableAmount, "Over full borrowable amount limit" );
        (,, uint256 MinCollateralLimit,,,,,) = ISwapHelper(SwapHelper).TradingInfo( BUSDContract,_swappedToken );
    ...
    function checkAutoStarting(address _trader) public view returns(bool){
        uint256 estSwapOut = ISwapHelper(SwapHelper).getSwapOut( BUSDContract, autoStarters[_trader].swappedToken, autoStarters[_trader].debtAmount );
